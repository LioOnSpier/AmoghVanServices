...
function generateReport(assets) {
  ...
  const totalExternal = Object.values(assets).reduce(
  ...
  if (totalExternal === 0) {
    console.log(
      "✅ NO EXTERNAL ASSETS FOUND! Your website is completely self-contained.",
    );
  } else {
    console.log(`📊 SUMMARY: Found ${totalExternal} external dependencies`);
    console.log(
      "💡 RECOMMENDATION: Download and host these assets locally for complete independence",
    );
  }
}
...
console.log("\n🚀 NEXT STEPS:");
console.log(
  "1. Run: bash scripts/download-fonts.sh (to download Google Fonts)",
);
